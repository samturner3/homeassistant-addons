import * as chuango from "./chuango";
...
const mqttMessageHandler =
  (commandTopic: string, callback: (payload: string) => Promise<void>) =>
...
//     const areaHandler = mqttMessageHandler(
//       commandTopic,
...
//     const doorHandler = mqttMessageHandler(
//       commandTopic,
//       async (payload: string) => {
...
//     const outputHandler = mqttMessageHandler(
//       commandTopic,
//       async (payload: string) => {
...
const startDevices = async (connectedClient: Client) => {
  const devices = chuango.discoverDevices(connectedClient);

  devices.then((devices: DeviceInfo[]) => {
    for (const device of devices) {
      console.info("Found device " + device.deviceID + " connecting...");
      connectedClient.connect(device).then((connection: DeviceConnection) => {
        console.info("Connected to device " + device.deviceID);
        console.debug({ device, connection });
      });
    }
  });

  // then do mapping to MQTT binary_sensor etc
...
export const connect = async (config: MqttConfig, connectedClient: Client) => {
  mqttConfig = config;

  await Promise.all([
    startDevices(connectedClient),
    // startControlAreas(),
    // startControlDoors(),
    // startControlOutputs(),
    // startControlInputs(),
